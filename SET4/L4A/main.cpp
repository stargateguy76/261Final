...
int main() {
 ...
 while (userIn=='Y') {
     ...
     cout << "Welcome one and all to a round of Rock, Paper, Scissors! (Enter P, R or S)"
          << endl; // ask the user for their choice of rock paper or scissors
     cin >> input;
     input = toupper(input);

     if (input ==
         'R') // if the user chooses rock, paper or scissors, print thier choice and set the win integer to is respective value
     {
     ...
     cout << "Computer choose: " << compChoice[computerNumber]
          << endl; // call that number as an index value for a string array carrying rock paper or scissors

     if (computerNumber ==
         win) // run comparisons between the computer choice and the user choice to determine who wins.
     {
         cout << "its a tie!" << endl;
     }
